...
- 配置文件会在完整写入临时文件后再安全替换，避免意外中断留下半截配置。
- 任务栏右键菜单修改显示模式或指标后，会在短暂防抖后自动保存。
- `Display format` 编辑器会显示格式校验结果，并保留实时预览。
- Settings 中的 `Diagnostics...` 仅在点击 `Re-detect` 时手动检测一次所有指标，显示简明的可用/不可用状态；不会在后台持续扫描。
- 未启用的指标不会执行对应硬件查询。
